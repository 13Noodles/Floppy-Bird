...
void draw(SDL_Renderer *renderer, struct GameObjects objects,
  ...
  draw_background(renderer, game_textures.background_spritesheet);
  draw_player(renderer, objects.player, game_textures.player_spritesheet);
  draw_obstacles(renderer, objects.obstacles,
                 game_textures.obstacle_spritesheet);
...
void draw_player(SDL_Renderer *renderer, struct Player player,
  ...
  SDL_RenderCopy(renderer, player_spritesheet.texture, &src_rect,
                  &player.rect);
}
...
void draw_obstacles(SDL_Renderer *renderer,
  ...
  for (size_t i = 0; i < MAX_OBSTACLES; ++i) {
    if (obstacles[i].is_used) {
      rect_from_sprite_id(obstacle_spritesheet,obstacles[i].sprite_id,&src_rect);
...
int rect_from_sprite_id(struct Spritesheet spritesheet, int sprite_id,SDL_Rect *rect) {
  int column = sprite_id % spritesheet.columns;
  int row = (sprite_id - column) / spritesheet.columns;
  if (row >= spritesheet.rows || sprite_id < 0) {
    printf("rect_from_sprite_id error : sprite_id=%d (expected a value between 0 and %d)\n",sprite_id,spritesheet.columns*spritesheet.rows);
    return -1;
  }
